...
class ActorTest {
    @Test
    fun testActorReceivesMessages() = runTest {
        val system = ActorSystemImpl()
        val actor = StringActor()
        val ref = system.createActor(logic = { actor })
        ref.send("hi")
        ref.send("there")
        delay(50)
        assertEquals(listOf("hi", "there"), actor.received)
        system.shutdown()
    }
}
...
class StringActor : Actor<String>() {
